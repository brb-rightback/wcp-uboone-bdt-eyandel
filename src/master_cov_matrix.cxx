...
void LEEana::CovMatrix::gen_xs_cov_matrix(int run, std::map<int, std::tuple<TH1F*, TH1F*, TH1F*, TH2F*, int> >& map_covch_hists, std::map<TString, std::tuple<TH1F*, TH1F*, TH1F*, TH2F*, int> >& map_histoname_hists, TVectorD* vec_mean,  TMatrixD* cov_xs_mat, TVectorD* vec_signal, TMatrixD* mat_R, int norm_sig_flag){
  ...
  for (size_t j = 0; j!=max_lengths.size(); j++){ // j: index of knobs
    ...
    for (int i=0;i!=nsize;i++){ // i: index of universes of a knob
      ...
      // }
      fill_xs_histograms(j, max_lengths.size(), acc_no, i, nsize,  map_passed_events, map_histoname_infos, map_no_histoname, map_histoname_hists, norm_sig_flag);
...
std::pair<std::vector<int>, std::vector<int> > LEEana::CovMatrix::get_events_weights_xs(TString input_filename, std::map<TString, std::set<std::tuple<float, float, std::vector<float>, std::vector<int>, std::set<std::tuple<int, float, bool, int> > > > >& map_passed_events, std::map<TString, double>& map_filename_pot, std::map<TString, std::tuple<int, int, int, TString>>& map_histoname_infos){
  ...
  for (size_t i=0;i!=T_eval->GetEntries();i++){
	...
	for (size_t j=0; j!= weight.xsr_scc_Fa3_SCC->size(); j++){
	    if (eval.weight_cv>0){
	      std::get<2>(event_info).at(acc_no+j) = (weight.xsr_scc_Fa3_SCC->at(j) -1.0);//- eval.weight_cv)/eval.weight_cv;
	  }else{
	    std::get<2>(event_info).at(acc_no+j) = 0;
	  }
	}
	...
	for (size_t j=0; j!= weight.xsr_scc_Fv3_SCC->size(); j++){
	    if (eval.weight_cv>0){
	      std::get<2>(event_info).at(acc_no+j) = (weight.xsr_scc_Fv3_SCC->at(j) - 1.0);//eval.weight_cv)/eval.weight_cv;
	  }else{
	    std::get<2>(event_info).at(acc_no+j) = 0;
	  }
	}
